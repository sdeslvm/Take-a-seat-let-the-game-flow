...
protocol TakeWebLoadable: AnyObject {
    var state: TakeWebStatus { get set }
    func setConnectivity(_ available: Bool)
}

/// Протокол для мониторинга прогресса загрузки
protocol TakeProgressMonitoring {
...
final class TakeWebLoader: NSObject, ObservableObject, TakeWebLoadable, TakeProgressMonitoring {
    // MARK: - Свойства

    @Published var state: TakeWebStatus = .standby
...
extension TakeWebLoader: WKNavigationDelegate {
