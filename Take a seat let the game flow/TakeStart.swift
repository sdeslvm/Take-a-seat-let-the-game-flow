...
protocol TakeGradientProviding {
...
final class TakeGradientContainerView: UIView, TakeGradientProviding {
...
struct TakeWebViewBox: UIViewRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: TakeWebLoader

    // MARK: - Координатор

    func makeCoordinator() -> TakeWebCoordinator {
        TakeWebCoordinator { [weak loader] status in
    ...
    private func setupContainerView(with webView: WKWebView) {
        let containerView = TakeGradientContainerView()
